...
if(isset($_POST['create_product']))
{
    ...
    if($prodmstrResult == 1)
    {
        ...
        if (!file_exists('../public/uploads/prodaddimages/')) {
            mkdir('../public/uploads/prodaddimages/', 0777, true); // Create directory recursively
        }
        
        if (!file_exists('../public/uploads/prod/')) {
            mkdir('../public/uploads/prod/', 0777, true); // Create directory recursively
        }
        ...
        if($createProduct > 0)
        {
            ...
            move_uploaded_file($_FILES['product_master_image']['tmp_name'], '../public/uploads/prod/' . $filemasname);
            move_uploaded_file($_FILES['products_image_one']['tmp_name'], '../public/uploads/prodaddimages/' . $fileaddonename);
            move_uploaded_file($_FILES['products_image_two']['tmp_name'], '../public/uploads/prodaddimages/' . $fileaddtwoname);
            move_uploaded_file($_FILES['products_image_three']['tmp_name'], '../public/uploads/prodaddimages/' . $fileaddthreename);
        }
    }
}
## ===*=== [I]NSERT PRODUCT DATA ===*=== ##
